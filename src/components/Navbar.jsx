  ...
  return (
    ...
    gap-6'>
      ...
      py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
        ...
        placeholder:text-[#716b94] text-white bg-transparent
        outline-none" />
        <div className="w-[72px] h-full rounded-[20px] bg-[#716b94] flex justify-center
        items-center cursor-pointer">
          <img src={search} alt="search" className="w-[15px] h-[15px]
          object-contain"/>
        </div>
      </div>
      {/* sm - only show in small device, hidden - it is usually hidden */}
      <div className="sm:flex hidden flex-row justify-end gap-4">
        <CustomButton
          btnType="button"
          title={address ? 'Create an auction' : 'Connect'}
          styles={address ? 'bg-[#716b94]' : 'bg-[#8c6dfd]'}
          handleClick={() => {
            // if address is available then navigate else trigger connect wallet function
            if(address) navigate('create-auction')
            else connect();
          }}
        />
      ...
      {/* Small screen navigation */}
          ...
          {/* Click Menu Button to toggle on/off menu components */}
          ...
          < div className = {
              ...
              {navlinks.map((link) => (
                ...
                >
                  <img
                    ...
                    object-contain ${isActive === link.
                      name ? 'grayscale-0' : 'grayscale'}`}
                  />

                  <p className={`ml-[20px] font-epilogue font-semibold text-[14px]
                    ${isActive === link.name ? 'text-[#c6c5d4]' : 'text-[#808191]'}`}>
                    {link.name}
                  </p>
                </li>
              ))}
            ...
            <div className="flex mx-4">
              <CustomButton
                btnType="button"
                title={address ? 'Create an auction' :
                'Connect'}
                styles={address ? 'bg-[#716b94]' : 'bg-[#8c6dfd]'}
                handleClick={() => {
                  if(address) navigate('create-auction')
